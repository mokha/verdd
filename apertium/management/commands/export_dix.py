...
def export_monodix(
    src_lang, tgt_lang, directory_path, ignore_file=None, *args, **kwargs
):
    raise NotImplementedError("Not implemented yet.")


def export_bidix(
    left_lang, right_lang, directory_path, ignore_file=None, *args, **kwargs
):
    checked = kwargs.get("approved", None)
    sdef_flag = kwargs.get("sdef")

    left_relations = Relation.objects.filter(type=TRANSLATION).filter(
        Q(lexeme_from__language=left_lang) & Q(lexeme_to__language=right_lang)
    )
    right_relations = Relation.objects.filter(type=TRANSLATION).filter(
        Q(lexeme_from__language=right_lang) & Q(lexeme_to__language=left_lang)
    )
    ...
    relations = (
        Relation.objects.filter(pk__in=_ids)
        .prefetch_related(
            Prefetch(
                "lexeme_from",
                queryset=Lexeme.objects.prefetch_related(
                    "miniparadigm_set", "lexememetadata_set"
                ),
            ),
            Prefetch(
                "lexeme_to",
    ...
    xml = render_to_string(
        "export/bidix_xml.html",
        {"alphabet": alphabet, "sdefs": sdefs, "relations": relations},
    )
    _filename = "apertium-{}-{}.{}-{}-{}.dix".format(
        left_lang, right_lang, left_lang, right_lang, time.strftime("%Y%m%d-%H%M")
    )
...
def export(left_lang, right_lang, directory_path, ignore_file=None, *args, **kwargs):
    ...
    return export_fn(
        left_lang, right_lang, directory_path, ignore_file, *args, **kwargs
    )
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            "--left",
            type=str,
            help="Three letter code of left language.",
        )
        parser.add_argument(
            "-r",
            "--right",
            ...
            nargs="?",
            default=None,
            help="Three letter code of right language.",
        )
        parser.add_argument(
            "-i",
            "--ignore",
