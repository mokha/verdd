...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "-d", "--database", type=str, help="The name of the database to backup."
        )
        parser.add_argument(
            "-p",
            "--path",
    ...
    def handle(self, *args, **options):
        ...
        if not database or not dir_path:
            ...
            return self.error_info(
                "Database %s not found in the configuration!" % database
            )
        ...
        if backup_database_path:
            self.success_info(
                "[+] Backup the database `%s` to %s"
                % (DB_SETTINGS["NAME"], backup_database_path)
            )
        else:
