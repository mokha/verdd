...
def export(src_lang, directory_path, *args, **kwargs):
    ...
    lexemes_data = (
        Lexeme.objects.filter(language=src_lang, checked=checked)
        .order_by("lexeme_lang")
        .values_list(
            "id",
            "lexeme",
            "pos",
    ...
    relations_data = relations.filter(lexeme_from__language=src_lang).values_list(
        "id", "lexeme_from", "lexeme_to"
    )
    ...
    _filename = "{}-{}-{}-csv-export.zip".format(
        time.strftime("%Y%m%d-%H%M%S"), src_lang, str(uuid.uuid4())[:5]
    )
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            "--dir",
            type=str,
            help="The directory path where to store the CSV file in.",
        )
        parser.add_argument(
            "-s",
            "--source",
