...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            "--model",
            type=str,
            help="The model to search for duplicates in. "
        ...
        parser.add_argument(
            ...
            type=str,
            nargs="+",
            help="The unique field names to find duplicates in.",
        )
        parser.add_argument(
            "--annotations", type=str, nargs="+", help="Extra annotations to obtain."
        )
        parser.add_argument(
            "--fields", type=str, nargs="+", default=("id",), help="Fields to display."
        )
        parser.add_argument(
            "--filters",
            type=str,
            nargs="+",
            default=tuple(),
            help="Filters to apply on duplicates.",
        )
        parser.add_argument(
            "-s",
            "--sort",
    ...
    def handle(self, *args, **options):
        try:
            ...
            duplicates = get_duplicate_objects(
                model=_model, annotations=annotations, unique_fields=unique_fields
            )
            ...
            for dd in duplicates:  # for each duplicate values
                ...
                for _d in d_objects:
                    dup_line.append(
                        obj_to_txt(_d, fields=fields, delimiter=delimiter)
