...
def read_xmls(xml_dir, lang_target="sms", lang_source="fin"):
    for filename in glob.glob(
        os.path.join(xml_dir + lang_target, "*.xml")
            ...
            for line in fp.readlines()[1:]:
                ...
                results.append(
                    (
                        row[2],
                        row[1],
                        {"semantics": row[3], "etymology": row[4], "sources": row[5]},
                    )
                )
        else:
            for line in fp:
...
def mediawiki_query(word, lexeme):
    ...
    if "error" not in r2:
        ...
        lines = re.findall(
            r'<span style="display:none" class="json_data">(.+?)</span>',
            lines,
        ...
        lines = map(
            lambda t: re.sub(
                r"<audio>(.+?)</audio>",
                "",
                html.unescape(t.replace("\\n", "")),
                re.UNICODE | re.IGNORECASE,
            ),
            lines,
        )  # audios aren't imported in the JSON properly
        try:
            ...
            for d in data:
                ...
                try:
                    mini_paradigm = html.unescape(d["morph"]["lg"]["mini_paradigm"])
                    tree = etree.parse(StringIO(mini_paradigm), parser=parser)
                    analysis = tree.findall("//analysis")
                    miniparam = [
                        (a.find("wordform").text, a.attrib["ms"].replace(".", "+"))
                        for a in analysis
                    ]
                except:
                    pass
...
def process_row(row, df, lang_source, lang_target):
    ...
    word_1_stripped = match_para.sub(
        "", word_1
    ).strip()  # remove any additional information
    word_1 = (
        word_1_stripped if word_1_stripped else word_1
    )  # in case the result is an empty string

    word_1_stripped = match_para.sub(
        "", word_2
    ).strip()  # remove any additional information
    word_2 = (
        word_1_stripped if word_1_stripped else word_2
    )  # in case the result is an empty string

    word_1_analysis = analyze(word_1, "fin")
    word_1_analysis = list(
        filter(lambda _a: _a[0] and "Hom" not in _a[0], word_1_analysis)
    )
    ...
    lexemes_1 = [
        create_lexeme(lexeme=word_1, pos=pos, language=lang_source, imported_from=df)
        for pos in word_1_analysis
    ]

    word_2_analysis = analyze(word_2, "sms")
    word_2_analysis = list(
        filter(lambda _a: _a[0] and "Hom" not in _a[0], word_2_analysis)
    )
    word_2_analysis = set(map(lambda wa: wa[0], word_2_analysis))
    if not word_2_analysis:
        word_2_analysis = [""]
    lexemes_2 = [
        create_lexeme(lexeme=word_2, pos=pos, language=lang_target, imported_from=df)
        for pos in word_2_analysis
    ]  # insert second word
    ...
    for w1 in lexemes_1:
        for w2 in lexemes_2:
            r = create_relation(
                w1, w2, notes, row_dict["teâttkäivv"]
            )  # create relation
    ...
    for w2 in lexemes_2:
        ...
        c = (
            list(
                filter(
                    lambda c: "contlex" in c and c["contlex"].startswith(pos + "_"),
                    contlex,
                )
            )
            if contlex
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                'Successfully imported the file "%s", with ID: %d' % (file_path, df.id)
            )
        )
