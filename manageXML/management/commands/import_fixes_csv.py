...
def process(rows):
    ...
    for row in rows:
        ...
        if not pos.strip():
            ...
            try:
                l = Lexeme.objects.get(
                    lexeme=row[2], language=language, pos__gt="", homoId=homoId
                )
                pos = l.pos
        ...
        try:
            ...
            l = Lexeme.objects.get(
                lexeme=row[2], language=language, pos=pos, homoId=homoId
            )
            log_change(
                original_lexeme.id,
                original_lexeme.lexeme,
        ...
        for w in row[3:]:
            try:
                _l = Lexeme.objects.get(
                    lexeme=w, pos=pos, language=language, homoId=homoId
                )
                log_change(_l.id, _l.lexeme, "NA_LEX", "Lexeme already exist")
            except:
                _l = create_lexeme(
                    lexeme=w, pos=pos, language=language, imported_from=df
                )
                ...
                log_change(
                    _l.id, _l.lexeme, "CREATE_LEX", "Created lexeme %s" % (_l.lexeme)
                )

            r, created = Relation.objects.get_or_create(
                lexeme_from=main_lexeme,
                lexeme_to=_l,
                type=VARIATION,
        ...
        for wrong_id in wrong_lexemes_ids:
            ...
            for r in relations:
                ...
                if main_l.language == "fin":
                    _r, created = Relation.objects.get_or_create(
                        lexeme_from=main_l, lexeme_to=r.lexeme_to, type=TRANSLATION
                    )
                else:
                    _r, created = Relation.objects.get_or_create(
                        lexeme_from=r.lexeme_from, lexeme_to=main_l, type=TRANSLATION
                    )
                if created:
                    log_change(
                        main_l.id,
                        main_l.lexeme,
                        "CREATED_TRANSLATION",
                        "Created translation: %s -> %s"
                        % (r.lexeme_from.lexeme, main_l.lexeme),
                    )
                if r.notes:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('Successfully imported the file "%s"' % (file_path,))
        )
