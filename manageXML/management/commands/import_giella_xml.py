...
def create_lexeme(ll: GiellaXML.Item, lang: Language, datafile: DataFile = None):
    ...
    for _k, _v in _filtered_attributes.items():
        ...
        _lmd, created = LexemeMetadata.objects.get_or_create(
            lexeme=_l, type=_metadata_type, text=_v
        )
...
def parseXML(filename, filepos):
    ...
    for e in g.elements:
        ...
        try:
            ...
            if (
                not _ll
            ):  # shouldn't happen but if it did, then we shouldn't get it there
                continue
            for mg in e.get("mg", []):
                ...
                for xg in mg.get("xg", []):  # examples
                    ...
                    for xt in xg.get("xt", []):
                        ...
                        re_src, created = RelationExample.objects.get_or_create(
                            relation=_r, text=x, language=gl
                        )

                        xtt = xt.text
                        re_tgt, created = RelationExample.objects.get_or_create(
                            relation=_r, text=xtt, language=langs[_lang]
                        )
                ...
                for defNative in mg.get("defNative", []):
                    if not defNative or not defNative.text:
                        continue
                    _lmd, created = LexemeMetadata.objects.get_or_create(
                        lexeme=_ll, type=DEF_NATIVE, text=defNative.text.strip()
                    )
...
class Command(BaseCommand):
    ...
    help = (
        "This command imports the content of a all Giella XML documents in a directory."
    )
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            "--dir",
            type=str,
            help="The directory containing XML files.",
        )
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        for filename in glob.glob(
            ...
            try:
                ...
                self.stderr.write(
                    self.style.ERROR("Error processing %s: %s" % (filename, str(err)))
                )
        self.stdout.write(
            self.style.SUCCESS("Successfully imported the files in %s." % (xml_dir,))
        )
