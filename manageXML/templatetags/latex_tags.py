...
nti_ids = read_first_ids_from(
    os.path.join(settings.BASE_DIR, "../local/data/not-to-inflect.csv")
)
...
def dictionary_entry(grouped_relation):
    ...
    translation_lemma_map = {"V": "+Inf", "N": "+N+Sg+Nom", "A": "+Sg+Nom", "Adv": ""}
    ...
    for r in relations:
        ...
        if translation.pos in translation_lemma_map:
            result = uralicApi.generate(
                (
                    ...
                    if translation.homoId > 0
                    else "" + translation.pos + translation_lemma_map[translation.pos]
                ),
        ...
        if translation.id not in nti_ids:  # ignore certain translations
                    ...
                    for i in range(len(queries)):
                        ...
                        try:
                            generated_MP_forms[MP_form].append(
                                synthetiser.lookup(queries[i])[0][0].split("@")[0]
                            )
                        except:
                            raise
                ...
                if translation.contlex and translation.pos in contelex_inflexType_cases:
                    for re_pattern, _inflections in contelex_inflexType_cases[
                        ...
                        if re.search(
                            re_pattern,
                            "{},{}".format(
                                translation.contlex, translation.inflexType_str()
                            ),
                        ):
            ...
            if (
                not inflections
                and translation.pos == "N"
                and re.match(r"[A-Z](.+)", translation.lexeme)
            ):
